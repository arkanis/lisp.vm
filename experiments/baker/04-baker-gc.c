...
Fixes the crashes of the previous segmented Baker GC by making the stack and indirect stack. Instead of containing atom
pointers it contains references to all the variables that contains atom pointers. During collect we can treat each of
those references like a root: Preserve the atom and patch the pointer on the C stack to the new memory address.

With that the GC seems to work well. Also remember: NO TEST COVERAGE! So anyone using this code with production in mind
can be declared stupid indeed (yes, this is addressed to anyone reading this code).
...
typedef struct atom_s atom_t, *atom_p, **atom_var_p;
...
void stack_push(atom_var_p atom_var) {
	if (sp >= stack_length) {
		// Stack full, grow it
...
void gc_collect_atom(atom_var_p atom, uint32_t level) {
#	ifdef GC_DEBUG
...
#	endif
	...
	// Also ignore NULL pointers in atom variables (e.g. in arrays that are just filled in with valid atoms).
	if (old_atom == NULL || old_atom == nil_atom || old_atom == true_atom || old_atom == false_atom)
		return;
...
**/
atom_p eval(atom_p atom, atom_p env, atom_var_p survivers[]) {
	switch(atom->type) {
		...
		case T_ARRAY:
			{
				...
				for(size_t i = 0; survivers[i] != NULL; i++)
					stack_push(survivers[i]);
				
				gc_collect_if_necessary();
				
				if (atom->array.len == 0) {
					result = error_atom("eval: array needs at least one elment to be evaled");
					goto end;
				}
				
				atom_p func = eval(atom->array.ptr[0], env, (atom_var_p[]){ NULL });
				if (func->type == T_ERROR) {
					result = func;
					goto end;
				}
				if ( !(func->type == T_BUILTIN || func->type == T_SYNTAX || func->type == T_LAMBDA) ) {
					result = error_atom("eval: only builtin, syntax and lambda atoms are allowed in the function slot");
					goto end;
				}
				...
				if (func->type == T_BUILTIN) {
					...
					// Zero out the individual atom pointer since we might get a collect while assigning the arguments.
					// In that case the collector can just ignore NULL pointers but random pointers won't work.
					args->array.ptr = gc_alloc_zeroed(args->array.len * sizeof(atom_p));
					
					for(size_t i = 1; i < atom->array.len; i++) {
						atom_p evaled_arg = eval(atom->array.ptr[i], env, (atom_var_p[]){ &func, &args, NULL });
					...
					for(size_t i = 0; i < args->array.len; i++) {
						if (args->array.ptr[i]->type == T_ERROR) {
							result = args->array.ptr[i];
							goto end;
						}
					}
					
					result = func->syntax(args, env);
				} else if (func->type == T_LAMBDA) {
					if ( func->lambda.args->array.len != atom->array.len - 1 ) {
						result = error_atom("eval: a lambda needs to be called with exactly as many args as defined");
						goto end;
					}
					...
					for(size_t i = 1; i < atom->array.len; i++) {
						...
						if (arg_name->type != T_SYM) {
							result = error_atom("eval: in the definition of an lambda an argument has to be a symbol");
							goto end;
						}
						
						atom_p evaled_arg = eval(atom->array.ptr[i], env, (atom_var_p[]){ &func, &lambda_env, &arg_name, NULL });
						if (evaled_arg->type == T_ERROR) {
							result = evaled_arg;
							goto end;
						}
					...
					result = eval(func->lambda.body, lambda_env, (atom_var_p[]){ &func, &lambda_env, NULL });
				}
...
atom_p syntax_set(atom_p args, atom_p env) {
	...
	hash_set(env, args->array.ptr[0]->sym, eval(args->array.ptr[1], env, (atom_var_p[]){ &args, &env, NULL }));
	return nil_atom;
}
...
atom_p syntax_if(atom_p args, atom_p env) {
	...
	atom_p cond = eval(args->array.ptr[0], env, (atom_var_p[]){ &args, &env, NULL });
	if (cond->type == T_FALSE || cond->type == T_NIL)
		return eval(args->array.ptr[2], env, (atom_var_p[]){ &args, &env, NULL });
	else
		return eval(args->array.ptr[1], env, (atom_var_p[]){ &args, &env, NULL });
}
...
// Stupidly evals the same code in an endless loop. Meant for profiling the interpreter.
atom_p syntax_endless(atom_p args, atom_p env) {
	while(true) {
		eval(args->array.ptr[0], env, (atom_var_p[]){ &args, &env, NULL });
	}
	return nil_atom;
}
...
int main() {
	...
	hash_set(base_env, "endless", syntax_atom(syntax_endless));
	...
	while (true) {
		...
		atom = eval(atom, base_env, (atom_var_p[]){ NULL });
