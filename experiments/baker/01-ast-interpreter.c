...
This is an AST interpreter with no memory management whatsoever. It is meant as a base for a just for fun experiment to
write a baker garbage collector.

- Right now we just allocate and never free.
