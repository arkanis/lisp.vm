...
void test_gc_alloc() {
	...
	lvm_atom_p num_atom = lvm_gc_alloc(lvm, LVM_T_NUM, 0, NULL);
	...
	size_t data_size = 13;
	void* data_ptr = NULL;
	lvm_atom_p str_atom = lvm_gc_alloc(lvm, LVM_T_STR, data_size, &data_ptr);
	...
	st_check_msg(data_ptr == (void*)lvm->gc.new_space.last + LVM_GC_REGION_SIZE - data_size, "str data wasn't allocated where it should be");
	
	// Fill up one space to force allocation in a new region
	while ( lvm->gc.new_space.last->free_bytes > 4*1024 ) {
		lvm_atom_p filler_atom = lvm_gc_alloc(lvm, LVM_T_STR, 4*1024, &data_ptr);
		st_check_not_null(filler_atom);
	}
	
	// This allocation should create a new region
	lvm_atom_p new_region_atom = lvm_gc_alloc(lvm, LVM_T_STR, 4*1024, &data_ptr);
	st_check_not_null(new_region_atom);
	st_check_not_null(data_ptr);
...
void test_gc_collect() {
	...
	lvm_atom_p atom_a = lvm_gc_alloc(lvm, LVM_T_NUM, 0, NULL);
	st_check_not_null(atom_a);
	atom_a->num = 13;
	
	const char* str_b = "hello atom b!";
	lvm_atom_p atom_b = lvm_gc_alloc(lvm, LVM_T_STR, strlen(str_b)+1, &data_ptr);
	st_check_not_null(atom_b);
	st_check_not_null(data_ptr);
	atom_b->str = data_ptr;
	strcpy(atom_b->str, str_b);
	
	lvm_atom_p atom_c = lvm_gc_alloc(lvm, LVM_T_PAIR, 0, NULL);
	...
	for(size_t i = 0; i < 1024 * 8; i++) {
		lvm_atom_p garbage_atom = lvm_gc_alloc(lvm, LVM_T_STR, 4*1024, &data_ptr);
